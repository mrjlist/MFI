...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):   
        ...
        reference_imgs = gr.Textbox(label="Original frames folder", lines=1, elem_id=self.elem_id("OFF"))
    ...
    def run(self, p, append_interrogation, reference_imgs, first_denoise, third_frame_image, color_correction_enabled, unfreeze_seed, loopback_source):
        ...
        file_OFF = os.listdir(reference_imgs)
        reference_imgs = [f"{reference_imgs}/{i}" for i in sorted(file_OFF)]
        
        loops = len(reference_imgs)
        
        print("Elements:-->", loops)
        for i in range(loops):
            print(reference_imgs[i])
        ...
        for n in range(batch_count):
            ...
            for i in range(loops):
                print("Frame generation:-->", reference_imgs[i])
